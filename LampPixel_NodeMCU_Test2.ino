...
#include <FastLED.h>
#include "LampPixel_NodeMCU_Test2.h"
...
void loop() {
  ...
  changeAnimation();
  //changeBrightness(); //Disable car qu'un port Analogique sur MCU
  FastLED.setBrightness(MAX_BRIGHTNESS); //Forcer Brightness a font 

    return;
  }


  //check if command sent by wifi else use  local command
  // Read which button was pressed on the Web Page
  Serial.print("Start read client\n");
 
  
  String request = client.readStringUntil('\r');
  Serial.print("End read client\n"); 

  
  if (request.indexOf("/xyTester=1") != -1) {
    Serial.print("xyTester=1\n");
    currentEffect = 0;
    wipeMatrices();
  }
  if (request.indexOf("/hueRotationEffect=1") != -1) {
    Serial.print("HUE Rotation selected\n");
    currentEffect = 1;
    wipeMatrices();
  }

  if (request.indexOf("/animatePacChase=1") != -1) {
    Serial.print("Pac Man Pink selected\n");
    currentEffect = 2;
    wipeMatrices();
  }


  if (request.indexOf("/animatePacman=1") != -1) {
    Serial.print("Pack Man selected\n");
    currentEffect = 3;
    wipeMatrices();
  }


  if (request.indexOf("/hue=1") != -1) {
    Serial.print("HUE selected\n");
    currentEffect = 4;
    wipeMatrices();
  }

   if (request.indexOf("/fireworks=1") != -1) {
    Serial.print("fireworks selected\n");
    currentEffect = 5;
    wipeMatrices();
  }

  if (request.indexOf("/firepit=1") != -1) {
    Serial.print("firepit selected\n");
    currentEffect = 6;
    wipeMatrices();
  }

  if (request.indexOf("/nothing=1") != -1) {
    Serial.print("nothing selected\n");
    currentEffect = 7;
    wipeMatrices();
  }

  if (request.indexOf("/test=1") != -1) {
    Serial.print("nothing selected\n");
    currentEffect = 8;
    wipeMatrices();
  }
...
client.println("<a href=\"/xyTester=1\"\"><button>xyTester</button></a>");
client.println("<a href=\"/hueRotationEffect=1\"\"><button>HUE Rotation</button></a>");
client.println("<a href=\"/animatePacChase=1\"\"><button>Pac Man Pink </button></a><br />"); 
client.println("<a href=\"/animatePacman=1\"\"><button>Pac Man </button></a><br />"); 
client.println("<a href=\"/hue=1\"\"><button>HUE </button></a><br />"); 
client.println("<a href=\"/fireworks=1\"\"><button>FireWork </button></a><br />"); 
client.println("<a href=\"/firepit=1\"\"><button>Firepit </button></a><br />"); 
client.println("<a href=\"/nothing=1\"\"><button>Nothing </button></a><br />"); 
client.println("<a href=\"/test=1\"\"><button>Test </button></a><br />"); 
...
void fireworks() {
  random16_add_entropy( rand() ); //random() en rand() pour NodeMCU
  CRGB sky1(0,0,2);
  CRGB sky2(2,0,2);
